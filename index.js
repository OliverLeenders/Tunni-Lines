...
class path {
    ...
    update_path(update_tunni) {
        ...
        if (this.same_side(this.start, this.end, this.C1, this.C2)) {
            // need to check legality of positions
            if ((this.is_left(this.start, this.end, this.C1) != this.is_left(this.start, this.C1, this.C2))
                && (this.is_left(this.end, this.start, C2) != this.is_left(this.end, this.C2, this.C1))) {
                // console.log("legal");
                let is = this.intersection(this.start, this.C1, this.C2, this.end);
                //console.log(is.to_string())
                this.is_ui_el
                    .attr("opacity", 1)
                    .attr("cx", is.x)
                    .attr("cy", is.y);
    ...
    drag = (e) => {
        ...
        if (sel_el) {
            ...
            if (sel_el.attr("id") == "start") {
                ...
                let C1_halfway_point = new point((e.x + this.start.x) / 2, (e.y + this.start.y) / 2);
                let C2_vector = new point(this.C2.x - this.end.x + C1_halfway_point.x, this.C2.y - this.end.y + C1_halfway_point.y);
                let C1_intersection = this.intersection(C1_halfway_point, C2_vector, this.start, this.C1);
                ...
                let C1_vector = new point(this.C1.x - this.start.x + C2_halfway_point.x, this.C1.y - this.start.y+ C2_halfway_point.y);
                let C2_intersection = this.intersection(C2_halfway_point, C1_vector, this.end, this.C2);
